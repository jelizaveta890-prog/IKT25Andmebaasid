...
as begin 
 ...
 --vaja teha päring, kus on töötajaid 2tk või tohkem 
...
update Employee set Gender = 'Female', DepartmentName = 'Payroll' 
where Id = 1
--ei luba tabelis korraga andmeid muuta, kui on tegemist CTE-ga

--kokkuvõtte CTE-st 
--1.kui CTE baseerub ühele tabelil, siis uuendus töötab
--2.kui CTE baseerub mitmel tabelil, siis tuleb veateade
--3.kui CTE baseerub mitmel ja tahame muuta ainult ühte tabelit,
--siis uuendus saab tehtud

--korduv CTE 
--CTE, mis iseendale viitab, kutsutakse korduvaks CTE-ks
--kui tahad näidata andmeid herarhiliselt
Create Table Employee
(
...
--kasutame left join, et näha kõiki töötajaid ja nende juhte
select Emp.Name as [Employee Name],
isnull(Manager.Name, 'SuperBoss') as [Manager Name]
...
--päring muudab unikaalsete veergudega väärtustega (India, US ja  UK) SalesCountory veerus
--omaette veergudeks koos veergudega SalesAmount liitmisega.
...
--pööramise ja grupeerimises järgi
...
--transactionid
--transactionid jälgib järgimise samme:
-- 1. Selle algus
-- 2. käivitab DB käske
-- 3. kontrollib vigu. Kui on viga, siis tagastab  algse oleku
...
--enamus juhtudel saab subqueri-t asendada JOIN-iga 
--teha päring  joini-ga, et saada müümata toode infot (left join)
...
--suqueryt saab subquwry sisse panna 
--subquery on alati sulgudes ja neid nimetakse sisemisteks päringuteks
...
--vastab rea seadistuse e suleb cursori
...
---tabeli info
---nimekiri süsteeemi objektidest 
...
--kui soovid erinevad  objektitüüpe vaadata, siis kasuta XTYPE süntaksit
...
begin
 ...
 -- tahame sama tabeli ära kustutada ja siis uuesti luua 
...
-- ühendab sihttabeli lähtetabeliga ja kasutab mõlemas tabelis ühist
-- koodinäide:
merge [TARGET] as T
using [SOURCE] as S
    on [JOIN_CONDITIONS]
...
--- mida vähem valvet lugemine tuleb, seda aeglasem
...
--- Lost update e kandunud uuendused 
select * from Inventory
